...
file = "/home/daniel/PycharmProjects/Fiscalia/InformeViolaciones/INFORMACIÓN VIOLACIÓN al 12 de Diciembre 2024 ok.xlsx"
datos = pd.read_excel(file, skiprows=1)
datos["AÑO"] = datos.iloc[:, 0].ffill().astype(int).astype(str)

ult_12_meses = datos.iloc[-15:-1]
...
plt.title("""Violaciones con detenido
...
ax = df_prod_sd.plot(kind="bar",
                     ...
                     figsize=(14, 6),
                     color=["yellowgreen", "cornflowerblue"])
plt.title("""Violaciones sin detenido
Últimos 12 meses""", fontsize = 25)
ax.legend(fancybox=True, framealpha=0.5, bbox_to_anchor=(1, -0.15))
ax2 = ax.twinx()
ax2.set_ylabel("Porcentaje")
